...
data class DebtInput(val name:String,val lenderName:String,val debtType:String,val originalPrincipal:Long,val currentPrincipal:Long,val interestRate:Double?,val interestType:String,val startDate:LocalDate,val maturityDate:LocalDate?,val paymentFrequency:String,val expectedPaymentAmount:Long,val nextDueDate:LocalDate,val note:String?,val receivedAmount:Long=0,val isNewLoan:Boolean=false,val remainingMonths:Int?=null,val receiveAccountId:String?=null,val totalPeriods:Int?=null,val paymentMode:String="principal_interest",val currentPeriodPaid:Boolean=false)
...
interface FinanceRepository {
    ...
    suspend fun updateIncomeSource(id:String,input:IncomeSourceInput):Result<Unit>
    suspend fun deleteIncomeSource(id:String):Result<Unit>
