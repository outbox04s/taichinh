...
fun FinanceApp(viewModel: FinanceViewModel = hiltViewModel()) {
    ...
    Scaffold(containerColor = Color.Transparent, bottomBar = {
        ...
        NavHost(nav, "overview", Modifier.padding(padding)) {
            composable("overview") { OverviewScreen(state, viewModel::refresh, viewModel::setPeriod, viewModel::setAccount, viewModel::setCustomRange, viewModel::addIncome, viewModel::linkIncome, { nav.navigate("settings") }, { nav.navigate("transaction/new") }, { navigateToMainTab("transactions") }, { navigateToMainTab("debts") }, { nav.navigate("reports/categories") }, { nav.navigate("fixed-expenses") }, { nav.navigate("income-sources") }) }
            composable("transactions") { TransactionsScreen(state, viewModel::refresh, viewModel::setSearch, viewModel::setType, viewModel::setSource, viewModel::setAccount, viewModel::setCategory, viewModel::deleteTransaction, { nav.navigate("transaction/transfer") }) { nav.navigate("transaction/new") } }
            ...
            composable("reports") { ReportsScreen(state, viewModel::refresh, { nav.navigate("fixed-expenses") }, { nav.navigate("reports/categories") }, viewModel::addIncome, viewModel::linkIncome, { nav.navigate("income-sources") }) }
            composable("reports/categories") { CategoryReportScreen(state, viewModel::refresh, { nav.popBackStack() }, viewModel::setPeriod, viewModel::setAccount, viewModel::setCustomRange) }
            composable("fixed-expenses") { FixedExpensesScreen(state, viewModel::refresh, { nav.popBackStack() }, { id, input, done -> viewModel.saveFixedExpense(id, input, done) }, { id, done -> viewModel.deleteFixedExpense(id, done) }) }
            composable("income-sources") { IncomeManagementScreen(state, { nav.popBackStack() }, viewModel::refresh, { id, input, done -> viewModel.saveIncome(id,input,done) }, { id, done -> viewModel.deleteIncome(id,done) }, viewModel::linkIncome) }
