...
create or replace function public.create_debt_v4(
 ...
 p_first_due_date date,p_principal_due_date date,p_payment_frequency text,p_receive_account_id uuid default null,p_note text default null
) returns uuid language plpgsql security definer set search_path='' as $$
declare v_user uuid:=auth.uid();v_debt uuid:=gen_random_uuid();v_due date:=p_first_due_date;
 v_current bigint;v_left bigint;v_amount bigint;v_index integer:=1;v_category uuid;
begin
 ...
 v_current:=case when p_is_new_loan or p_payment_mode='interest_only' then p_principal else p_expected_payment_amount*p_remaining_periods end;
 insert into public.debts(id,user_id,name,lender_name,debt_type,original_principal,current_principal,interest_rate,interest_type,start_date,maturity_date,payment_frequency,expected_payment_amount,next_due_date,status,note,received_amount,is_new_loan,remaining_months,total_periods,payment_mode)
 values(v_debt,v_user,trim(p_name),nullif(trim(p_lender_name),''),'loan',p_principal,v_current,null,case when p_has_interest then 'monthly' else 'none' end,current_date,p_principal_due_date,p_payment_frequency,p_expected_payment_amount,p_first_due_date,'active',p_note,case when p_is_new_loan then p_principal else 0 end,p_is_new_loan,p_remaining_periods,p_total_periods,p_payment_mode);
 if p_payment_mode='interest_only' then
  ...
  if v_left>0 then raise exception 'Monthly amount and remaining periods do not cover total payable';end if;
 end if;
 if p_is_new_loan then
...
grant execute on function public.create_debt_v4(text,text,bigint,boolean,boolean,text,integer,integer,bigint,date,date,text,uuid,text) to authenticated;
commit;
